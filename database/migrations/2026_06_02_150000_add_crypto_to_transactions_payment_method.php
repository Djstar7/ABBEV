...
/**
 * Ajoute `crypto` aux moyens de paiement de la table `transactions`.
 *
 * Sans cela, toute transaction crypto (paiement BTC/ETH/USDT via NOWPayments)
 * échoue sur la contrainte CHECK de `payment_method`.
 *
 * On part de l'enum courant (incluant `apple_iap`, ajouté précédemment) et on
 * y ajoute `crypto`. SQLite ne supporte pas la modification d'une contrainte
 * CHECK : on recrée donc les colonnes enum (compatible SQLite & MySQL).
 */
