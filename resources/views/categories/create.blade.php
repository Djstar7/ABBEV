...
@section('title', 'Nouvelle catégorie - Movie Dashboard')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-sky-primary">Nouvelle catégorie</h1>
    </div>
        ...
        <form action="{{ route('categories.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea name="description"
                          id="description"
                          rows="4"
                          class="w-full bg-dark-bg border border-gray-600 rounded-md px-4 py-2 text-white focus:outline-none focus:border-sky-primary">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
